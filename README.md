...
- **Phase 2 (cache): in progress.** `Memory`/`Cache` split with separate icache/dcache, true-LRU replacement, write-back with dirty-bit tracking, and byte/half/word access including cross-line boundaries all implemented and verified. Remaining: flush dirty lines to memory on halt, and cycle-cost/hit-rate accounting.
